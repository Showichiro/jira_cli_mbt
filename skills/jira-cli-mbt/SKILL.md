...
Prefer the canonical noun-verb forms: `issue list|get|create|update|transition|comment|assign|search`, `project list`, and `field list|get`. Legacy aliases such as `issues`, `create`, `search`, `projects`, and `fields` still work during the migration period.
...
- `issue list` defaults to `assignee = currentUser() ORDER BY updated DESC`.
